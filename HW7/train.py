...
model = AutoModelForQuestionAnswering.from_pretrained(pretrained_model).to(device)
...
# for par in [train_paragraphs, dev_paragraphs, test_paragraphs]:
#     new_tokens = set(list(''.join(par))) - set(tokenizer.vocab.keys())
#     tokenizer.add_tokens(list(new_tokens))
# print(f'New tokenizer size: {len(tokenizer)}')

# model.resize_token_embeddings(len(tokenizer))
# You can safely ignore the warning message as tokenized sequences will be further processed in dataset __getitem__ before passing to model
...
class QA_Dataset(Dataset):
    def __init__(self, split, questions, tokenized_questions, tokenized_paragraphs):
        ...
        self.doc_stride = self.max_paragraph_len // 4
    ...
    def __getitem__(self, idx):
        ...
        if self.split == "train":
            ...
            prefix_len = random.randint (0, self.max_paragraph_len)
...
logging_step = len(train_set) // train_batch_size // 10

learning_rate = 1e-5
optimizer = AdamW(model.parameters(), lr=learning_rate)

total_step = len(train_set) // train_batch_size * num_epoch
warmup_ratio = 0
scheduler = get_linear_schedule_with_warmup(optimizer, warmup_ratio * total_step, total_step)
    ...
    if validation:
        ...
        with torch.no_grad():
            ...
            if dev_acc > best_acc:
                ...
                best_acc = dev_acc
                print(f"Validation | Epoch {epoch + 1} | acc = {dev_acc / len(dev_loader):.3f} -> best")
