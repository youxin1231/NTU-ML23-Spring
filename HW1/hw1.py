...
save_pred(preds, './output/pred.csv')
...
learning_rate=[round(float(x),2) for x in np.linspace(start=1e-5, stop=1e-2, num=10)]
...
xgbmodel = RandomizedSearchCV(estimator = xgbmodel, param_distributions=random_grid,
                              n_iter=200, cv=3, verbose=2, random_state=2023, n_jobs=-1)
...
if not os.path.exists(os.path.dirname('./output')):
    os.makedirs(os.path.dirname('./output'))
save_pred(predicted, './output/pred_xgb.csv')

valid_pred = xgbmodel.predict(x_valid)
