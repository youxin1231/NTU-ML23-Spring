...
def mifgsm(model, x, y, loss_fn, epsilon=epsilon, alpha=alpha, num_iter=40, decay=0.5):
    ...
    for i in range(num_iter):
        ...
        momentum = decay * momentum + grad / grad.abs().sum()
        x_adv = x_adv + alpha * momentum.sign()
        x_adv = torch.clamp(x_adv, x - epsilon, x + epsilon)

    return x_adv

def dim_mifgsm(model, x, y, loss_fn, epsilon=epsilon, alpha=alpha, num_iter=60, decay=0.9, prob = 0.9):
    if torch.rand(1) <= prob:
        # Resize
        resize_factor = torch.rand(1) * 0.3 + 0.7
        new_size = (int(x.size(2) * resize_factor), int(x.size(3) * resize_factor))
        x_resized = transforms.Resize(new_size)(x)

        # Padding
        padding_left = torch.randint(0, 32 - new_size[0] + 1, (1,))
        padding_right = x.size(2) - new_size[0] - padding_left
        padding_top = torch.randint(0, 32 - new_size[1] + 1, (1,))
        padding_bottom = x.size(3) - new_size[1] - padding_top
        x_padded = transforms.Pad((padding_left, padding_top, padding_right, padding_bottom))(x_resized)

        x_adv = x_padded.detach().clone().to(device)
...
def main():
    ...
    print(f'dim_fgsm_acc = {dim_mifgsm_acc:.5f}, dim_fgsm_loss = {dim_mifgsm_loss:.5f}')
